...
def create_graph(llm1, llm2):
    ...
    def printNode(state: AgentState) -> dict:
        if state.get("verbose"):
            print("[TRACE] Entering printNode node")
        return {}
    ...
    graph_builder.add_conditional_edges(
        ...
        route_after_input,      # Routing function that examines state
        {
            ...
            "llmNode": "llmNode",  # Any input -> proceed to LLM
            END: END                  # Quit command -> terminate graph
        }
    )

    graph_builder.add_edge("llmNode", "call_llm")
    graph_builder.add_edge("llmNode", "call_qwen")

    graph_builder.add_edge("call_llm", "printNode")
